    ...
    public class OrderingIntegrationEventService : IOrderingIntegrationEventService
    {
        ...
        public async Task PublishEventsThroughEventBusAsync(Guid transactionId)
        {
            ...
            foreach (var logEvt in pendingLogEvents)
            {
                ...
                {
                    _logger.LogError(ex, "ERROR publishing integration event: {IntegrationEventId} from {AppName}", logEvt.EventId,
                        Program.AppName);
        ...
        {
            _logger.LogInformation("----- Enqueuing integration event {IntegrationEventId} to repository ({@IntegrationEvent})", evt.Id,
                evt);
