    ...
    public class IntegrationEventsScenarios
    {
        ...
        public async Task Post_update_product_price_and_catalog_and_basket_list_modified()
        {
            ...
            var res = await basketClient.PostAsync(
                ...
                new StringContent(JsonSerializer.Serialize(basket), Encoding.UTF8, "application/json")
            );
            ...
            var pRes = await catalogClient.PutAsync(CatalogScenariosBase.Put.UpdateCatalogProduct,
                new StringContent(ChangePrice(itemToModify, newPrice, originalCatalogProducts), Encoding.UTF8, "application/json"));
        ...
        private async Task<BasketItem> GetUpdatedBasketItem(decimal newPrice, int productId, string userId, HttpClient basketClient)
        {
            ...
            while (continueLoop && counter < 20)
            {
                ...
                var basketUpdated = JsonSerializer.Deserialize<CustomerBasket>(await basketGetResponse.Content.ReadAsStringAsync(),
